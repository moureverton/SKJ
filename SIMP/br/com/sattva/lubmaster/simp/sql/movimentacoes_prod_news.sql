...
FROM(
  SELECT
    ...
    CASE WHEN(A.USOPROD = 'M')
         THEN (SELECT CLASSE FROM AD_OPERACAOANP OPER WHERE OPER.CODOPER = C.AD_CODOPERANPMP)
         ELSE (SELECT CLASSE FROM AD_OPERACAOANP OPER WHERE OPER.CODOPER = C.AD_CODOPERANP)
    END AS CLASS_OPERACAO,
    CASE WHEN(A.USOPROD = 'M') THEN C.AD_CODOPERANPMP
         ELSE (CASE WHEN C.AD_CODOPERANP IS NOT NULL THEN C.AD_CODOPERANP ELSE C.AD_CODOPERANPAGENTENREGULADO END)
    END AS OPERACAO,
  ...
  WHERE COALESCE(C.AD_ATUALIZA_EST_SIMP,'N') = 'S'
    ...
    AND B.CODEMP = ?
    AND (CASE WHEN (E.USOPROD = '2') THEN '1'
              ELSE (CASE WHEN (E.USOPROD = 'V') THEN '2'
                         ELSE (CASE WHEN (A.USOPROD = 'M')
                                    THEN (SELECT CLASSE FROM AD_OPERACAOANP OPER WHERE OPER.CODOPER = C.AD_CODOPERANPMP)
                                    ELSE (SELECT CLASSE FROM AD_OPERACAOANP OPER WHERE OPER.CODOPER = C.AD_CODOPERANP) END)
                   END)
        END) =
        (CASE WHEN (A.USOPROD = 'M')
              THEN (SELECT CLASSE FROM AD_OPERACAOANP OPER WHERE OPER.CODOPER = C.AD_CODOPERANPMP)
...
GROUP BY
  ...
  VLRUNIT, RECIPIENTE_GLP, CHAVE_NFE_CTE, OPER_TOTALIZADOR, IDIPROC
UNION ALL
SELECT
  IDIPROC,NUNOTA,
  LISTAGG(SEQUENCIA,',') AS SEQUENCIA,
...
FROM(
  SELECT
    ...
    (CASE WHEN C.AD_CODOPERANP IS NOT NULL THEN C.AD_CODOPERANP ELSE C.AD_CODOPERANPAGENTENREGULADO END) AS OPERACAO,
    COALESCE(F.AD_CODEMP_ANP,'0') AS AD_CODEMP_ANP,
    COALESCE(F.AD_CODINSTALACAO_ANP,'0') AS COD_INST_EMP,
    COALESCE(D.AD_CODINSTALACAO_ANP,'0') AS COD_INST_PAR,
    TO_CHAR(COALESCE(E.CODANP,0)) AS CODPROD_ANP,
